...
| Game database catalogs | `/regions`, `/heroes`, `/items`, `/monsters`, `/quests`, `/parties`, `/factions`, `/npcs`, `/locations`, `/classes`, `/abilities`                    | Browse game-facing entity collections.                                   |
...
Top-level navigation v1 has no dropdowns. It uses direct links only:
...
| Language | Header button | Shows the current language, for example `EN`, `RU`, or `ZH`.                        |

Do not add top-nav dropdowns in v1. Detail pages and secondary catalogs are
reached from catalogs, search, related links, or section subnav.
